-- 1. Pin search_path on the internal email queue helpers (all calls are schema-qualified).
...
-- 2. Internal queue plumbing must not be callable from the Data API.
...
-- 3. Webhook delivery log: explicitly exclude anonymous (guest) sessions.
