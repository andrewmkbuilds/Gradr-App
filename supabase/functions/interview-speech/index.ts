...
serve(async (req) => {
  ...
  try {
    ...
    if (!apiKey) return json({ error: "tts_unavailable", reason: "Voice is not configured." }, 503);
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        console.error("elevenlabs request failed", String(e));
        res = null;
      }
      if (res && res.ok) break;
    ...
    if (!res || !res.ok || !res.body) {
      ...
      console.error("elevenlabs tts failed", status, detail.slice(0, 400));
      return json({ error: "tts_failed", status, details: detail.slice(0, 300) }, status === 401 ? 502 : status);
    }
