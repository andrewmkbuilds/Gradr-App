...
/**
 * Auth action URL passthrough. Auth emails MUST use the dynamic, single-use URL
 * generated by the auth provider (token_hash / type / redirect_to intact). We never
 * rewrite or reconstruct it; the only fallback is the sign-in page, used when a
 * payload somehow arrives without a URL so the email never renders a dead link.
 */
export const authActionUrl = (url?: string) =>
  url && /^https?:\/\//i.test(url) ? url : `${SITE_URL}/auth`;
