...
export const affiliateApplicationSchema = z.object({
  ...
  promotion_plan: text,
  why_join: text,
  social_links: z.unknown().catch(null),
  payout_details: z.unknown().catch(null),
  admin_notes: text,
...
export const discountRuleSchema = z.object({
  ...
  active: z.boolean().catch(true),
  advertised: z.boolean().catch(false),
  requires_verification: z.boolean().catch(false),
  stackable: z.boolean().catch(false),
  applicable_plans: z.array(z.string()).catch([]),
  applicable_intervals: z.array(z.string()).catch([]),
  max_redemptions: z.coerce.number().nullish().catch(null).transform((v) => v ?? null),
  redemption_count: z.coerce.number().catch(0),
