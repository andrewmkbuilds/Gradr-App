...
import { discountRuleSchema, parseAdminRows } from "@/lib/admin/schemas";
...
export default function AdminDiscounts() {
  ...
  const rules = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw error;
      return parseAdminRows(discountRuleSchema, data, "discount_rules").rows;
    },
  });
  ...
  const review = useMutation({
    mutationFn: async (input: { id: string; status: VerificationStatus; reason?: string }) => {
      await adminRpcOrThrow(
        ...
        { requestId: newRequestId(`review-verification-${input.id}`) },
      );
    },
    onSuccess: () => {
